...
class TestReconnectOnConnect(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        self.minimal_config = Config.from_dict({
            "events": {
    ...
    async def test_onconnect_fires_on_initial_connect(self) -> None:
        ...
        with (
            ...
            mock_config_class.load.return_value = self.minimal_config
        ...
        on_connect_calls = [c for c in spec_calls if c[0] == self.minimal_config.events.OnConnect]
    ...
    async def test_onconnect_fires_after_reconnect(self) -> None:
        ...
        with (
            ...
            on_connect_initial = [c for c in spec_calls if c[0] == self.minimal_config.events.OnConnect]
            ...
            on_connect_after = [c for c in spec_calls if c[0] == self.minimal_config.events.OnConnect]
